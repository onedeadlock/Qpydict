...
#if (!defined(MM_SIMD_FLAG) || !MM_SIMD_FLAG) && !defined(MODE_GENERIC)
#include <stdint.h>
...
#        // No check for Aarch64, set `+simd` in the compiler flags as appropriate.
#    else
#        // set full neon 16 lanes as default in Aarch64 mode or for missing int64.
#        if HAVE_ARM64 || defined(MODE_NEON128) || !defined(INT64_MAX)
#           // can also be set explicitly for Armv7-A/Aarch32 mode with `-DMODE_NEON128`
