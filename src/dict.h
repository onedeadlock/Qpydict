...
/** @dict_for_each_mask: visit every set entry in dict; mask is handled externally and must be set to zero or call @dict_for_each_next_mask before it can visit the next group else it loops infinitely */
#define dict_for_each_mask_(v, j, m, cmp, _s, next, ...) \
    for (size_t j=0; (v.i < v.size); next(v.i))\
        for (mask_t m=(v.mm=mm_load(v.grp+v.i), cmp(v.mm)); m;)

/** @dict_for_each_cmp: the top block for @dict_for_each_cmp_set_ent; visits entries by probing. It can be called independent of its main block though (an infinte loop trap) */ 
